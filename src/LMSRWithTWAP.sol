...
interface ILMSRMarketMaker {
    function calcMarginalPrice(uint8 outcomeTokenIndex) external view returns (uint256 price);
    function atomicOutcomeSlotCount() external view returns (uint256);
    function trade(int[] memory outcomeTokenAmounts, int collateralLimit) external returns (uint256 cost);
}

contract LMSRWithTWAP {
    ...
    constructor(address _marketMaker) {
        ...
        uint256 outcomes = marketMaker.atomicOutcomeSlotCount();
    ...
    function updateCumulativeTWAP() public {
        ...
        for (uint8 i = 0; i < length; i++) {
            ...
            uint256 currentPrice = marketMaker.calcMarginalPrice(i);
            if (currentPrice == 0) {
                continue;
            }
    ...
    function trade(int[] memory outcomeTokenAmounts, int collateralLimit) public {
        ...
        //marketMaker.trade(outcomeTokenAmounts, collateralLimit);
