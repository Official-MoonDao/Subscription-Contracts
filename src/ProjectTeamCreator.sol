...
contract ProjectTeamCreator is Ownable {
    ...
    function createProjectTeam(string memory adminHatURI, string memory managerHatURI, string memory memberHatURI, string calldata name, string calldata description, string calldata image, uint256 quarter, uint256 year, uint256 MDP, string calldata proposalIPFS, string calldata upfrontPayments, address lead, address[] memory members) external onlyOwner() returns (uint256 tokenId, uint256 childHatId) {
        ...
        table.insertIntoTable(tokenId, name, description, image, quarter, year, MDP, proposalIPFS, "", "", "", "", upfrontPayments, 1, 0);
    }
